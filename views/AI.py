...
interpreter = tf.lite.Interpreter(model_path="views/hello_quantized_model.tflite")
...
if selected == 'Kamera':
    ...
    if picture:
        if st.button("Prediksi"):
            # Memberikan input ke interpreter
            ...
            # Tentukan kelas berdasarkan threshold 0.5 (sesuai binary classification)
            if prediction > 0.5:
                result = "Anorganik"
...
if selected == "Photo":
    ...
    if uploaded_file is not None:
        ...
        if st.button("Prediksi"):
            ...
            # Memberikan input ke interpreter (TensorFlow Lite)
            input_index = input_details[0]['index']
            interpreter.set_tensor(input_index, img_array)
